...
const OurproductScreen = (category) => {
  ...
  // useEffect(() => {
  //   // Assuming 'products' is an array of product objects and we're using the first product as an example
  //   if (products.length > 0) {
  //     const firstProductColors = JSON.parse(products[0].ProductSizeList[0].ProductColors || '[]');

  //     firstProductColors.forEach(color => {
  //       const productImages = JSON.parse(color.ProductImagePath || '[]');
  //       const srNo1Image = productImages.find(image => image.SrNo === 1);
        
  //       if (srNo1Image) {
  //         console.log(srNo1Image.ImagePath);
  //       } else {
  //         console.error('Image with SrNo 1 not found');
  //       }
  //     });
  //   }
  // }, [products]);
  ...
  const handleCategoryChange = (category) => {
    if (!categoryChangeCooldown) {
      const newCategoryId = category.CategoryId === selectedCategoryId ? null : category.CategoryId;
                                ...
                                <picture className="w-100 d-block">
                                  ...
                                  >
                                    <img
                                      ...
                                      src={imagePaths[product.ProductId] ? imagePaths[product.ProductId][0] : add_to_icon}
